...
(function (ReVIEW) {
    (function (Parse) {
        function parse(input) {
            ...
            ReVIEW.visit(root, {
                visitDefaultPre: function (ast) {
                    if (ast.ruleName === Parse.RuleName.Ulist) {
        ...
        function transform(rawResult) {
            ...
            var rule = Parse.RuleName[rawResult.syntax];
            if (typeof rule === "undefined") {
                throw new Parse.ParseError(rawResult, "unknown rule: " + rawResult.syntax);
            }
            switch (rule) {
                case Parse.RuleName.Chapter:
                    return new Parse.ChapterSyntaxTree(rawResult);
                case Parse.RuleName.BlockElement:
                    return new Parse.BlockElementSyntaxTree(rawResult);
                case Parse.RuleName.Headline:
                    return new Parse.HeadlineSyntaxTree(rawResult);
                case Parse.RuleName.InlineElement:
                    return new Parse.InlineElementSyntaxTree(rawResult);
                case Parse.RuleName.BracketArg:
                case Parse.RuleName.BraceArg:
                    return new Parse.ArgumentSyntaxTree(rawResult);
                case Parse.RuleName.UlistElement:
                    return new Parse.UlistElementSyntaxTree(rawResult);
                case Parse.RuleName.OlistElement:
                    return new Parse.OlistElementSyntaxTree(rawResult);
                case Parse.RuleName.DlistElement:
                    return new Parse.DlistElementSyntaxTree(rawResult);
                case Parse.RuleName.ContentText:
                case Parse.RuleName.BlockElementContentText:
                case Parse.RuleName.InlineElementContentText:
                case Parse.RuleName.ContentInlineText:
                case Parse.RuleName.SinglelineComment:
                    return new Parse.TextNodeSyntaxTree(rawResult);

                case Parse.RuleName.Chapters:
                case Parse.RuleName.Paragraphs:
                case Parse.RuleName.Contents:
                case Parse.RuleName.BlockElementContents:
                case Parse.RuleName.InlineElementContents:
                case Parse.RuleName.ContentInlines:
                case Parse.RuleName.Ulist:
                case Parse.RuleName.Olist:
                case Parse.RuleName.Dlist:
                    return new Parse.NodeSyntaxTree(rawResult);

                case Parse.RuleName.Start:
                case Parse.RuleName.Paragraph:
                case Parse.RuleName.DlistElementContent:
                    return new Parse.NodeSyntaxTree(rawResult);
                case Parse.RuleName.Content:
                case Parse.RuleName.BlockElementContent:
                case Parse.RuleName.InlineElementContent:
                case Parse.RuleName.SinglelineContent:
                case Parse.RuleName.ContentInline:
                    return transform(rawResult.content);
                default:
                    return new Parse.SyntaxTree(rawResult);
            }
        }
...
(function (ReVIEW) {
    (function (Build) {
        ...
        var DefaultAnalyzer = (function () {
            ...
            DefaultAnalyzer.prototype.init = function (book) {
                var _this = this;
                this.book = book;
                ...
                book.parts.forEach(function (part) {
                    part.chapters.forEach(function (chapter) {
                        ReVIEW.visit(chapter.root, {
                            ...
                            visitHeadlinePre: function (node) {
                                _this.headline(chapter.process, "hd", node);
                            },
                            visitBlockElementPre: function (node) {
                                _this.block(chapter.process, node.name, node);
                            },
                            visitInlineElementPre: function (node) {
                                _this.inline(chapter.process, node.name, node);
                            }
                        });
                    });
                });
            ...
            DefaultAnalyzer.prototype.headline = function (process, name, node) {
                var label = null;
                if (node.tag) {
                    label = node.tag.arg;
            ...
            DefaultAnalyzer.prototype.block = function (process, name, node) {
            ...
            DefaultAnalyzer.prototype.inline = function (process, name, node) {
            ...
            DefaultAnalyzer.prototype.checkArgsLength = function (process, node, expect) {
            ...
            DefaultAnalyzer.prototype.constructReferenceTo = function (process, node, value, targetSymbol, separator) {
            ...
            DefaultAnalyzer.prototype.block_list = function (process, node) {
            ...
            DefaultAnalyzer.prototype.inline_list = function (process, node) {
            ...
            DefaultAnalyzer.prototype.inline_hd = function (process, node) {
                if (node.childNodes.length !== 1) {
                    process.error(t("compile.body_string_only"));
                }
                process.addSymbol({
    ...
    (function (Parse) {
            ...
            NodeSyntaxTree.prototype.processChildNodes = function (content) {
                ...
                if (Array.isArray(content)) {
                    content.forEach(function (rawResult) {
                        var tree = Parse.transform(rawResult);
                        if (tree) {
        ...
        var ChapterSyntaxTree = (function (_super) {
            ...
            function ChapterSyntaxTree(data) {
                ...
                this.headline = Parse.transform(this.checkObject(data.headline)).toHeadline();
        ...
        var HeadlineSyntaxTree = (function (_super) {
            ...
            function HeadlineSyntaxTree(data) {
                ...
                if (data.label !== "") {
                    this.label = Parse.transform(this.checkObject(data.label)).toArgument();
                }
                if (data.tag !== "") {
                    this.tag = Parse.transform(this.checkObject(data.tag)).toArgument();
                }
                this.caption = Parse.transform(this.checkObject(data.caption)).toNode();
            }
        ...
        var BlockElementSyntaxTree = (function (_super) {
            ...
            function BlockElementSyntaxTree(data) {
                ...
                this.args = this.checkArray(data.args).map(function (data) {
                    return Parse.transform(data).toArgument();
                });
            }
        ...
        var UlistElementSyntaxTree = (function (_super) {
            ...
            function UlistElementSyntaxTree(data) {
                _super.call(this, data);
                this.level = this.checkNumber(data.level);
                this.text = Parse.transform(this.checkObject(data.text));
        ...
        var DlistElementSyntaxTree = (function (_super) {
            ...
            function DlistElementSyntaxTree(data) {
                _super.call(this, data);
                this.text = Parse.transform(this.checkObject(data.text));
                this.content = Parse.transform(this.checkObject(data.content));
            }
    ...
    var Controller = (function () {
        ...
        Controller.prototype.process = function () {
            ...
            this.config.validators.forEach(function (validator) {
                validator.init(book, _this.config.builders);
            });
